...
bool Run::init()
{
	...
	pDebugRpcServer = Server::ServerBuilder::build(getServerPortFromParameters());
...
{
	unsigned int result = Server::DebugRpcServer::DEFAULT_PORT;
...
bool Run::start()
{
	...
	{
			{TEXT("Kernel debugger service"), (LPSERVICE_MAIN_FUNCTION)serviceEntry},
			{NULL, NULL}
	};
...
bool Run::startAsApp()
{
	...
	try
	{
		pDebugRpcServer->start();
		std::cout << "Press 'enter' to exit..." << std::endl;
		std::string exitVar;
		std::cin >> exitVar;

		pDebugRpcServer->stop();
